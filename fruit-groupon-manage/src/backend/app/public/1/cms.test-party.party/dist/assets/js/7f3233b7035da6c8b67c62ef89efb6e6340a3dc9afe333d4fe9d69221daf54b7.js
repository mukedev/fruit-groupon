...
(function () {
  ...
  function title(query) {
    if (env.site.path === 'static/articles') {
      if (query.search !== undefined) {
        document.title = "".concat(query.search, " | Search | ").concat(env.base.title);
      } else if (query.categoryId !== undefined) {
        document.title = "".concat(query.categoryName, " | Category | ").concat(env.base.title);
      } else if (query.tagId !== undefined) {
        document.title = "".concat(query.tagName, " | Tag | ").concat(env.base.title);
      }
    } else if (env.site.path === 'static/comments') {
      document.title = "Comments | ".concat(env.base.title);
    }
  }

  function menuActive(query) {
    var categoryId = query.categoryId || env.article && env.article.atomCategoryId;

    if (categoryId) {
      var link = $(".category-".concat(categoryId));
      link.parents('li').addClass('active');
    }
  }

  function breadcrumb(query) {
    var $container = $('.breadcrumb-nav');
    ...
    if (env.site.path === 'static/articles') {
      if (query.search !== undefined) {
        ...
        $('.parent', $container).text('Category');
        $('.current', $container).text(query.categoryName);
      } else if (query.tagId !== undefined) {
        $('.parent', $container).text('Tag');
        $('.current', $container).text(query.tagName);
      }
    } else if (env.site.path === 'static/comments') {
      $('.parent', $container).text('Comment');
      $('.current', $container).text('All');
    } else if (env.site.path === 'main/article') {
      $('.parent', $container).html("<a href=\"".concat(util.url('static/articles.html'), "?categoryId=").concat(env.article.atomCategoryId, "&categoryName=").concat(encodeURIComponent(env.article.atomCategoryName), "\">").concat(util.escapeHtml(env.article.atomCategoryName), "</a>"));
    ...
    util.loadMore({
      ...
      onFetch: function onFetch(_ref2) {
        ...
        var options = {
          language: env.language && env.language.current,
          where: {},
          orders: [['p.sticky', 'desc'], ['a.createdAt', 'desc']],
          page: {
            index: index
          },
          mode: 'default'
        ...
        if (categoryId) {
          ...
          options.orders = [['p.sticky', 'desc'], ['p.sorting', 'asc'], ['a.createdAt', 'desc']];
        }

        if (tagId) {
          options.tag = tagId;
          options.orders = [['p.sticky', 'desc'], ['a.createdAt', 'desc']];
        }

        if (search) {
          options.where['q.html'] = {
            val: search,
            op: 'like'
          };
          options.mode = 'search';
      ...
      onParse: function onParse(item) {
        var sticky = !item.sticky ? '' : '<i class="fas fa-thumbtack"></i> ';
        var audio = !item.audioFirst ? '' : '<i class="fas fa-music"></i> ';
        var attachment = item.attachmentCount === 0 ? '' : '<i class="fas fa-paperclip"></i> ';
        var mediaUrl = item.imageFirst || item.audioCoverFirst;
        var media = !mediaUrl ? '' : "\n<a class=\"media-right\" target=\"_blank\" href=\"".concat(util.url(item.url), "\">\n<img class=\"media-object\" src=\"").concat(util.combineImageUrl(mediaUrl, 125, 100), "\">\n</a>\n");
        var category = categoryId === undefined ? "<a target=\"_blank\" href=\"".concat(util.url('static/articles.html'), "?categoryId=").concat(item.atomCategoryId, "&categoryName=").concat(encodeURIComponent(item.atomCategoryName), "\"><span class=\"num category\">").concat(util.escapeHtml(item.atomCategoryName), "</span></a>") : '';
        var tagsText = '';
        var tags = item.atomTags ? JSON.parse(item.atomTags) : null;
        ...
        if (tags && tags.length > 0) {
          ...
          for (var i = 0; i < tags.length; i++) {
            var _tagId = tags[i];
            var tagName = util.sidebar.getTagName(_tagId);
            tagsText += "<a target=\"_blank\" href=\"".concat(util.url('static/articles.html'), "?tagId=").concat(_tagId, "&tagName=").concat(encodeURIComponent(tagName), "\"><span class=\"num tag\">").concat(util.escapeHtml(tagName), "</span></a>");
          }

          tagsText += '';
        }

        var stat = "\n<div class=\"title stat no-parse\" data-article-id=\"".concat(item.atomId, "\">\n").concat(category, "\n<span class=\"num date\">").concat(util.formatDateTime(item.createdAt), "</span>\n<i class=\"fas fa-eye\"></i><span class=\"num readCount\">").concat(item.readCount, "</span>\n<i class=\"fas fa-heart\"></i><span class=\"num starCount\">").concat(item.starCount, "</span>\n<a target=\"_blank\" href=\"").concat(util.url(item.url), "#comments\"><i class=\"fas fa-comment\"></i><span class=\"num commentCount\">").concat(item.commentCount, "</span></a>\n").concat(tagsText, "\n</div>\n");
        return "\n<li>\n<div class=\"article-list-section\">\n<div class=\"article-list-content\">\n".concat(media, "\n<h4 class=\"article-list-title\">").concat(sticky).concat(audio).concat(attachment, "<a target=\"_blank\" href=\"").concat(util.url(item.url), "\">").concat(util.escapeHtml(item.atomName), "</a></h4>\n<p>").concat(util.escapeHtml(item.description) || item.summary, "</p>\n</div>\n").concat(stat, "\n</div>\n</li>\n");
      }
    });
  }
  ...
  function _fixContainerSite($navbar, $containerSite, $footer) {
    ...
    var _onScroll = function _onScroll() {
      ...
      var height = $containerSite.outerHeight();
